...
export default function MatrixTooltip({ active, payload }: MatrixTooltipProps) {
  if (active && payload && payload.length) {
    ...
    const qConfig = QUADRANT_CONFIG[data.quadrant as keyof typeof QUADRANT_CONFIG] || QUADRANT_CONFIG['Fillers'];
    ...
    return (
      <div className="bg-white p-0 border border-slate-200 shadow-xl rounded-lg z-50 min-w-[240px] overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex items-start justify-between bg-slate-50/50">
           ...
           <div className={`shrink-0 inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full border ${qConfig.bg} ${qConfig.color} ${qConfig.border}`}>
        ...
        <div className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
                <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                     <span className="block text-[10px] uppercase font-black tracking-widest text-slate-400 mb-1">Impact</span>
                ...
                <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                     <span className="block text-[10px] uppercase font-black tracking-widest text-slate-400 mb-1">Effort</span>
                      <div className="flex items-end">
             ...
             <div className="pt-2 border-t border-slate-100 grid grid-cols-2 gap-x-4 text-xs">
                 <div>
                     <span className="block text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Lead</span>
                     <span className="text-slate-700 font-medium truncate block">{data.owner}</span>
                </div>
                 <div className="text-right">
                     <span className="block text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Est. ROI</span>
